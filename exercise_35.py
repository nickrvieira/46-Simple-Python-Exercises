...
# English alphabet acrophonically (Alfa for A, Bravo for B, etc.)
#  so that critical combinations of letters (and numbers)
# can be pronounced and understood by those who transmit and receive voice messages by radio
# or telephone regardless of their native language,
...
# On a mac, you have access to the system TTS (TextToSpeech) as follows:
...
#  a float indicating the length of the pause between each spoken ICAO word,
# and a float indicating the length of the pause between each  word spoken.

import os, time


def speak_icao(message, pause_icao_word, pause_word):
